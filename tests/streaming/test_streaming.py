...
from litelm import Delta, ModelResponseStream, StreamingChoices
...
async def test_streaming_handles_space_correctly():
    ...
    with mock.patch("litelm.acompletion", side_effect=gpt_4o_mini_stream):
...
async def test_stream_listener_returns_correct_chunk_chat_adapter():
    ...
    with mock.patch("litelm.acompletion", side_effect=completion_side_effect):
        program = dspy.streamify(
            MyProgram(),
            stream_listeners=[
                dspy.streaming.StreamListener(signature_field_name="answer"),
...
async def test_stream_listener_returns_correct_chunk_json_adapter():
    ...
    with mock.patch(
        "litelm.acompletion", new_callable=AsyncMock, side_effect=[gpt_4o_mini_stream_1(), gpt_4o_mini_stream_2()]
    ):
        program = dspy.streamify(
            MyProgram(),
            stream_listeners=[
...
async def test_stream_listener_returns_correct_chunk_chat_adapter_untokenized_stream():
    ...
    with mock.patch("litelm.acompletion", new_callable=AsyncMock, side_effect=[gemini_stream_1(), gemini_stream_2()]):
        program = dspy.streamify(
            MyProgram(),
            stream_listeners=[
                dspy.streaming.StreamListener(signature_field_name="answer"),
...
async def test_stream_listener_missing_completion_marker_chat_adapter():
    ...
    with mock.patch("litelm.acompletion", side_effect=incomplete_stream):
        program = dspy.streamify(
            MyProgram(),
            stream_listeners=[
                dspy.streaming.StreamListener(signature_field_name="answer"),
...
async def test_status_message_non_blocking():
    ...
    with mock.patch("litelm.acompletion", new_callable=AsyncMock, side_effect=[dummy_tool]):
        with dspy.context(lm=dspy.LM("openai/gpt-4o-mini", cache=False)):
            output = program(question="why did a chicken cross the kitchen?")
            timestamps = []
            async for value in output:
...
async def test_streaming_allows_custom_streamable_type():
    ...
    with mock.patch("litelm.acompletion", side_effect=stream):
        with dspy.context(
...
async def test_streaming_with_citations():
    ...
    with mock.patch("litelm.acompletion", return_value=citation_stream()):
        program = dspy.streamify(
            MyProgram(),
            stream_listeners=[
                dspy.streaming.StreamListener(signature_field_name="answer"),
...
async def test_chat_adapter_simple_pydantic_streaming():
    ...
    with mock.patch("litelm.acompletion", side_effect=chat_stream):
        with dspy.context(lm=dspy.LM("openai/gpt-4o-mini", cache=False), adapter=dspy.ChatAdapter()):
...
async def test_chat_adapter_nested_pydantic_streaming():
    ...
    with mock.patch("litelm.acompletion", side_effect=nested_stream):
        with dspy.context(lm=dspy.LM("openai/gpt-4o-mini", cache=False), adapter=dspy.ChatAdapter()):
...
async def test_chat_adapter_mixed_fields_streaming():
    ...
    with mock.patch("litelm.acompletion", side_effect=mixed_stream):
        with dspy.context(lm=dspy.LM("openai/gpt-4o-mini", cache=False), adapter=dspy.ChatAdapter()):
...
async def test_json_adapter_simple_pydantic_streaming():
    ...
    with mock.patch("litelm.acompletion", side_effect=json_stream):
        with dspy.context(lm=dspy.LM("openai/gpt-4o-mini", cache=False), adapter=dspy.JSONAdapter()):
...
async def test_json_adapter_bracket_balance_detection():
    ...
    with mock.patch("litelm.acompletion", side_effect=complex_json_stream):
        with dspy.context(lm=dspy.LM("openai/gpt-4o-mini", cache=False), adapter=dspy.JSONAdapter()):
...
async def test_json_adapter_multiple_fields_detection():
    ...
    with mock.patch("litelm.acompletion", side_effect=multi_field_stream):
        with dspy.context(lm=dspy.LM("openai/gpt-4o-mini", cache=False), adapter=dspy.JSONAdapter()):
...
async def test_streaming_reasoning_model():
    ...
    with mock.patch("litelm.acompletion", side_effect=reasoning_stream):
...
async def test_stream_listener_empty_last_chunk_chat_adapter():
    ...
    with mock.patch("litelm.acompletion", side_effect=mock_stream):
        program = dspy.streamify(
            predict,
            stream_listeners=[
                dspy.streaming.StreamListener(signature_field_name="reasoning"),
...
async def test_streaming_reasoning_fallback():
    ...
    with mock.patch("litelm.acompletion", side_effect=non_reasoning_stream):
        with mock.patch("litelm.supports_reasoning", return_value=False):
            program = dspy.streamify(
                MyProgram(),
                stream_listeners=[
                    dspy.streaming.StreamListener(signature_field_name="reasoning"),
