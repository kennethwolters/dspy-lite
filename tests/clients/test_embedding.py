...
# Mock response format similar to litelm's embedding response.
...
def test_litelm_embedding(cache):
    ...
    with patch("litelm.embedding") as mock_litelm:
        # Configure mock to return proper response format.
        mock_litelm.return_value = MockEmbeddingResponse(mock_embeddings)

        # Create embedding instance and call it.
        embedding = Embedder(model, caching=True)
        result = embedding(inputs)

        # Verify litelm was called with correct parameters.
        # Because we disable the litelm cache, it should be called with caching=False.
        mock_litelm.assert_called_once_with(model=model, input=inputs, caching=False)
...
async def test_async_embedding():
    ...
    with patch("litelm.aembedding") as mock_litelm:
        # Configure mock to return proper response format.
        mock_litelm.return_value = MockEmbeddingResponse(mock_embeddings)
...
def test_call_caching_false_overrides_instance_true(cache):
    ...
    inputs = ["hello"]
    with patch("litelm.embedding") as mock_litelm:
        mock_litelm.return_value = MockEmbeddingResponse([[0.1, 0.2, 0.3]])
        embedding = Embedder(model, caching=True)
        embedding(inputs)
