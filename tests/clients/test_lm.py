...
def test_disabled_cache_skips_cache_key(monkeypatch):
    ...
    try:
        with (
            ...
            monkeypatch.setattr(litelm, "completion", fake_completion)
...
def test_lm_wraps_litelm_errors_with_metadata():
    lm = dspy.LM("openai/gpt-4o-mini")
    response = mock.Mock()
    response.status_code = 429
    response.headers = {"x-request-id": "req-123", "retry-after": "2.5"}

    error = litelm.RateLimitError(message="too many requests", llm_provider="openai", model="gpt-4o", response=response)
    wrapped = lm._wrap_litelm_exception(error)
...
def test_lm_wraps_litelm_context_window_error():
    lm = dspy.LM("openai/gpt-4o-mini")
    error = litelm.ContextWindowExceededError(message="too long", llm_provider="openai", model="gpt-4o")
    wrapped = lm._wrap_litelm_exception(error)
...
def test_lm_wraps_unknown_boundary_error_as_unexpected_error():
    lm = dspy.LM("openai/gpt-4o-mini")
    wrapped = lm._wrap_litelm_exception(RuntimeError("local boundary failure"))
...
def test_lm_preserves_existing_lm_error_without_self_cause():
    ...
    with mock.patch("dspy.clients.lm.litelm_completion", side_effect=error):
...
async def test_lm_preserves_existing_lm_error_without_self_cause_async():
    ...
    with mock.patch("dspy.clients.lm.alitelm_completion", side_effect=error):
        with pytest.raises(dspy.LMRateLimitError) as exc_info:
...
def test_reasoning_model_token_parameter():
    test_cases = [
        ...
        ("azure/gpt-5.4-chat", False),
        ("azure/gpt-5.chat", False),  # Real Azure deployment name
        ("openai/gpt-4", False),
...
def test_lm_supports_reasoning_with_litelm_capability_api():
...
def _direct_lm_case(lm_kind: str, outputs: list[str]):
    ...
    if lm_kind == "current_lm":
        patcher = mock.patch(
            "dspy.clients.lm.litelm_completion",
...
async def test_async_lm_call():
    ...
    with patch("litelm.acompletion") as mock_acompletion:
...
async def test_async_lm_call_with_cache(tmp_path):
    ...
    with mock.patch("dspy.clients.lm.alitelm_completion") as mock_alitelm_completion:
        mock_alitelm_completion.return_value = ModelResponse(
            choices=[Choices(message=Message(content="answer"))], model="openai/gpt-4o-mini"
        )
        mock_alitelm_completion.__qualname__ = "alitelm_completion"
        ...
        assert mock_alitelm_completion.call_count == 2
...
def test_responses_api():
    ...
    with mock.patch("litelm.responses", autospec=True, return_value=api_response) as dspy_responses:
        lm = dspy.LM(
...
def test_lm_replaces_system_with_developer_role():
    with mock.patch("dspy.clients.lm.litelm_responses_completion", return_value={"choices": []}) as mock_completion:
        lm = dspy.LM(
            "openai/gpt-4o-mini",
            cache=False,
            model_type="responses",
...
def test_responses_api_tool_calls(litelm_test_server, provider_fields):
    api_base, _ = litelm_test_server
    base_tool_call = {
        "type": "function_call",
        "name": "get_weather",
...
def test_responses_api_cache_hit_preserves_outputs_and_skips_usage(tmp_path):
    ...
    try:
        with mock.patch("litelm.responses", autospec=True, return_value=api_response) as responses:
...
def test_responses_api_joins_multiple_text_outputs():
    ...
    with mock.patch("litelm.responses", autospec=True, return_value=api_response):
...
def test_reasoning_effort_responses_api():
    ...
    with mock.patch("litelm.responses", return_value=make_response([])) as mock_responses:
        lm = dspy.LM(
...
def test_call_reasoning_model_with_chat_api():
    ...
    with mock.patch("litelm.completion", return_value=mock_response) as mock_completion:
        with mock.patch("litelm.supports_reasoning", return_value=True):
...
async def test_responses_api_with_none_usage_async():
    ...
    with mock.patch("litelm.aresponses", autospec=True, return_value=api_response):
        lm = dspy.LM(
            model="openai/gpt-5-mini",
            model_type="responses",
            cache=False,
...
async def test_streaming_passes_headers_correctly():
    ...
    with mock.patch("dspy.settings") as mock_settings:
        ...
        with mock.patch("litelm.acompletion") as mock_acompletion:
            ...
            with mock.patch("litelm.stream_chunk_builder", return_value={}):
                await stream_fn()

            # Verify headers were passed to litelm.acompletion
            mock_acompletion.assert_called_once()
...
def test_lm_responses_passes_hosted_tools_through_unchanged():
    ...
    with mock.patch("litelm.responses", autospec=True, return_value=response) as responses:
        lm = dspy.LM("openai/dspy-test-model", model_type="responses", cache=False)
