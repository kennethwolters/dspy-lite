...
def test_litelm_response_roundtrip(tmp_path):
    cache = _make_cache(str(tmp_path), set())
...
def test_text_completion_response_roundtrip(tmp_path):
    cache = _make_cache(str(tmp_path), set())
    response = {"id": "cmpl-test", "choices": [{"text": "Hello", "index": 0, "finish_reason": "stop"}], "model": "m"}
    cache["k"] = response
    result = cache["k"]
...
def test_embedding_response_roundtrip(tmp_path):
    cache = _make_cache(str(tmp_path), set())
    response = {"data": [{"embedding": [0.1, 0.2], "index": 0, "object": "embedding"}], "model": "m"}
    cache["k"] = response
    result = cache["k"]
...
def test_tool_call_response_roundtrip(tmp_path):
    """ModelResponse with tool calls uses nested litelm types."""
    cache = _make_cache(str(tmp_path), set())
    response = ModelResponse(
        id="chatcmpl-tool",
        choices=[{
...
def test_all_cached_lm_types_roundtrip():
    ...
    test_values = [
        ...
        {"data": [{"embedding": [0.1], "index": 0, "object": "embedding"}], "model": "m"},
    ]
