...
def test_json_adapter_does_not_read_local_file_from_lm_image_output(tmp_path, monkeypatch):
    secret = b"TOP-SECRET-AWS-KEY=AKIA_DEADBEEF_1234567890"
    secret_path = tmp_path / "SECRET_victim.txt"
    secret_path.write_bytes(secret)

    class ImageOutput(dspy.Signature):
        result_image: dspy.Image = dspy.OutputField()

    completion = f'{{"result_image": {{"url": "{secret_path}"}}}}'

    def fail_open(*args, **kwargs):
        pytest.fail("LM output parsing attempted to read the local secret")

    monkeypatch.setattr("builtins.open", fail_open)
    with pytest.raises(ValueError, match=r"Image\.from_path"):
        dspy.JSONAdapter().parse(ImageOutput, completion)
