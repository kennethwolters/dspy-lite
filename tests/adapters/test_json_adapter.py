...
def test_json_adapter_with_structured_outputs_does_not_mutate_original_signature():
    ...
    dspy.configure(lm=dspy.LM(model="openai/gpt-4o"), adapter=dspy.JSONAdapter())
    program = dspy.Predict(TestSignature)
    with mock.patch("litelm.completion"):
        program(input1="Test input")
...
def test_json_adapter_on_pydantic_model():
    ...
    with mock.patch("litelm.completion") as mock_completion:
        ...
        # Check that litelm.completion was called exactly once
        mock_completion.assert_called_once()
...
async def test_json_adapter_on_pydantic_model_async():
    ...
    with mock.patch("litelm.acompletion") as mock_completion:
        mock_completion.return_value = ModelResponse(
            choices=[
                Choices(
                    message=Message(
        ...
        # Check that litelm.acompletion was called exactly once
        mock_completion.assert_called_once()
...
def test_json_adapter_json_mode_no_structured_outputs():
    ...
    with (
        mock.patch("litelm.completion") as mock_completion,
        mock.patch("litelm.get_supported_openai_params") as mock_get_supported_openai_params,
        mock.patch("litelm.supports_response_schema") as mock_supports_response_schema,
    ):
...
async def test_json_adapter_json_mode_no_structured_outputs_async():
    ...
    with (
        mock.patch("litelm.acompletion") as mock_acompletion,
        mock.patch("litelm.get_supported_openai_params") as mock_get_supported_openai_params,
        mock.patch("litelm.supports_response_schema") as mock_supports_response_schema,
    ):
        # Call a model that allows json but not structured outputs
...
def test_json_adapter_toolcalls_no_native_function_calling():
    ...
    with mock.patch("dspy.adapters.json_adapter._get_structured_outputs_response_format") as mock_structured:
        # Patch litelm.completion to return a dummy response
        with mock.patch("litelm.completion") as mock_completion:
            mock_completion.return_value = ModelResponse(
...
def test_json_adapter_with_responses_api():
    ...
    program = dspy.Predict(TestSignature)
    with mock.patch("litelm.responses", autospec=True, return_value=api_response) as mock_responses:
