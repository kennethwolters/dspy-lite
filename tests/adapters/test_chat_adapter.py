...
def test_chat_adapter_format_exact_messages_with_citations_output_demo():
    ...
    expected_messages = [{"role": "system",
      "content": 'Your input fields are:\n'
                 ...
                 "particularly Anthropic's Citations API through LiteLM.\\n    Citations include the "
...
def test_null_content_raises_adapter_parse_error():
    ...
    with dspy.context(lm=lm):
        with mock.patch("litelm.completion", return_value=response):
...
def test_responses_model_native_tool_calling_round_trips_tool_only_output():
    ...
    with mock.patch("litelm.supports_function_calling", return_value=True), mock.patch(
        "litelm.responses", return_value=api_response
    ) as responses:
