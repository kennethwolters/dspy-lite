...
def test_require_stub_uses_install_hint_for_litelm(monkeypatch):
    import importlib.util
    import sys

    dist = _detect_dspy_dist()
    find_spec = importlib.util.find_spec
    monkeypatch.delitem(sys.modules, "litelm", raising=False)
    monkeypatch.setattr(importlib.util, "find_spec", lambda module: None if module == "litelm" else find_spec(module))

    stub = require("litelm", feature="dspy.LM")
    with pytest.raises(ImportError) as exc_info:
        _ = stub.something
    assert f"{dist}[litelm]" in str(exc_info.value)
