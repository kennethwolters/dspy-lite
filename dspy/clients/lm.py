...
import dspy
from dspy.clients._litelm import get_litelm, is_litelm_context_window_error
...
def _get_litelm():
    return get_litelm(feature="dspy.LM")


def _is_openai_reasoning_model(model: str) -> bool:
    model_family = model.split("/")[-1].lower() if "/" in model else model.lower()
    matches_reasoning_family = re.match(
        r"^(?:o[1345](?:-(?:mini|nano|pro))?(?:-\d{4}-\d{2}-\d{2})?|gpt-5(?:[.-].*)?)$",
        model_family,
    ) is not None
    return matches_reasoning_family and re.search(r"[.-]chat(?:[.-]|$)", model_family) is None
...
class LM(BaseLM):
        ...
        **kwargs,
        ...
        Args:
            model: The model to use. This should be a string of the form
                `"llm_provider/llm_name"` supported by LiteLM. For example,
                `"openai/gpt-4o"`.
    ...
    def _get_initial_kwargs(self, *, temperature, max_tokens, **kwargs) -> dict[str, Any]:
        # Override BaseLM's default kwargs shape for LiteLM/model-family-specific token parameters.
    ...
    def supports_function_calling(self) -> bool:
        return _get_litelm().supports_function_calling(model=self.model)

    @property
    def supports_reasoning(self) -> bool:
        return _get_litelm().supports_reasoning(self.model)

    @property
    def supports_response_schema(self) -> bool:
        return _get_litelm().supports_response_schema(model=self.model, custom_llm_provider=self._provider_name)

    @property
    def supported_params(self) -> set[str]:
        params = _get_litelm().get_supported_openai_params(model=self.model, custom_llm_provider=self._provider_name)
    ...
    def _get_cached_completion_fn(self, completion_fn, cache):
        ...
        litelm_cache_args = {"no-cache": True, "no-store": True}

        return completion_fn, litelm_cache_args

    def _wrap_litelm_exception(self, exc: Exception) -> LMError:
        """Convert exceptions raised at the LiteLM boundary into DSPy LM exceptions."""
        ...
        if is_litelm_context_window_error(exc):
            return ContextWindowExceededError(message=message or "Context window exceeded", **metadata)

        exc_cls = _lm_error_class_from_litelm_exception(exc) or _lm_error_class_from_status(status)
    ...
    def forward(
        ...
        **kwargs
        ...
        LiteLM/provider exceptions are wrapped in DSPy's structured LM error
        ...
        if self.model_type == "chat":
            completion = litelm_completion
        elif self.model_type == "text":
            completion = litelm_text_completion
        ...
        try:
            ...
            if isinstance(e, LMError):
                raise
            raise self._wrap_litelm_exception(e) from e
    ...
    async def aforward(
        ...
        **kwargs,
        ...
        if self.model_type == "chat":
            completion = alitelm_completion
        elif self.model_type == "text":
            completion = alitelm_text_completion
        elif self.model_type == "responses":
    ...
    def _check_truncation(self, results):
        if self.model_type == "responses":
            return
        choices = results.choices if hasattr(results, "choices") else results["choices"]
...
def _get_stream_completion_fn(
    ...
    async def stream_completion(request: dict[str, Any], cache_kwargs: dict[str, Any]):
        response = await _get_litelm().acompletion(
            cache=cache_kwargs,
            stream=True,
            headers=headers,
            **request,
        ...
        return _get_litelm().stream_chunk_builder(chunks)
...
def litelm_completion(request: dict[str, Any], num_retries: int, cache: dict[str, Any] | None = None):
...
def litelm_text_completion(request: dict[str, Any], num_retries: int, cache: dict[str, Any] | None = None):
    cache = cache or {"no-cache": True, "no-store": True}
    request = dict(request)
    request.pop("rollout_id", None)
    ...
    return _get_litelm().text_completion(
        cache=cache,
...
async def alitelm_completion(request: dict[str, Any], num_retries: int, cache: dict[str, Any] | None = None):
    cache = cache or {"no-cache": True, "no-store": True}
    request = dict(request)
    request.pop("rollout_id", None)
    headers = _add_dspy_identifier_to_headers(request.pop("headers", None))
    ...
    if stream_completion is None:
        return await _get_litelm().acompletion(
            cache=cache,
            num_retries=num_retries,
            retry_strategy="exponential_backoff_retry",
            headers=headers,
...
async def alitelm_text_completion(request: dict[str, Any], num_retries: int, cache: dict[str, Any] | None = None):
    cache = cache or {"no-cache": True, "no-store": True}
    request = dict(request)
    request.pop("rollout_id", None)
    model = request.pop("model").split("/", 1)
    ...
    return await _get_litelm().atext_completion(
        cache=cache,
        model=f"text-completion-openai/{model}",
        api_key=api_key,
        api_base=api_base,
...
def litelm_responses_completion(request: dict[str, Any], num_retries: int, cache: dict[str, Any] | None = None):
    cache = cache or {"no-cache": True, "no-store": True}
    request = dict(request)
    request.pop("rollout_id", None)
    headers = request.pop("headers", None)
...
async def alitelm_responses_completion(request: dict[str, Any], num_retries: int, cache: dict[str, Any] | None = None):
    cache = cache or {"no-cache": True, "no-store": True}
    request = dict(request)
    request.pop("rollout_id", None)
    headers = request.pop("headers", None)
    ...
    return await _get_litelm().aresponses(
        cache=cache,
        num_retries=num_retries,
        retry_strategy="exponential_backoff_retry",
        headers=_add_dspy_identifier_to_headers(headers),
...
def _safe_litelm_exception_class(name: str) -> type[Exception] | None:
    cls = getattr(_get_litelm(), name, None)
    return cls if isinstance(cls, type) and issubclass(cls, Exception) else None


def _lm_error_class_from_litelm_exception(exc: Exception) -> type[LMError] | None:
    ...
    for litelm_name, dspy_cls in mappings:
        litelm_cls = _safe_litelm_exception_class(litelm_name)
        if litelm_cls is not None and isinstance(exc, litelm_cls):
            return dspy_cls
    return None
...
# Best-effort LiteLM/provider exception metadata extraction.
#
# LiteLM exception metadata is not exposed as a single stable typed shape across providers, exception classes, and
# LiteLM versions. Keep the defensive getattr-based extraction localized here so the rest of DSPy sees structured
# DSPyError metadata.
