...
class Embedder:
    ...
    1. Hosted embedding models (e.g. OpenAI's text-embedding-3-small) via litelm integration
    ...
    litelm to handle the API calls and caching.
    ...
    Args:
        model: The embedding model to use. This can be either a string (representing the name of the hosted embedding
            model, must be an embedding model supported by litelm) or a callable that represents a custom embedding
            model.
